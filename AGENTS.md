...
**Category tiles — bento grid, each with its own soft color identity.** Below the hero card, a 2-column bento grid (Rent, Groceries, Utilities, Bills, Kids Tuition, extensible). Per the updated color guidance in Section 2, give each category tile a consistent, soft signature tint (not just a colored icon on an otherwise identical dark card) — e.g. a muted color wash across the tile background or a soft glow at one edge — so the grid reads as varied and lively rather than a uniform dark grid with only icon color changing. Each tile: icon, name, allocation in dual currency, tappable to edit via the same keypad entry.

**"Add Category" must actually work.** Tapping it currently does nothing — fix the missing handler. It should open a sheet with three ways in:
- A short curated preset list of common household categories not yet added (Transportation, Insurance, Health & Medical, Internet & Phone, Entertainment, Savings) — reliable, no API call needed, this is the default view.
- An "✨ Suggest with AI" action that makes a single on-demand call to a free-tier LLM (Gemini's free tier is a good fit here, especially since a Google AI Pro account is already in place) sending the categories already added plus brief household context, asking for a handful of relevant additional suggestions. This is cheap and free-tier-safe specifically because it's one call, on-demand, only when this sheet is opened — not a recurring or per-keystroke call.
- Manual entry: name, icon (from a small built-in icon set), and allocation amount.

**Relocation Estimator.** Keep this visually distinct from the recurring category tiles — it's a one-time planning tool, not an ongoing budget line. Wire it to actually compute a suggestion: given Adults + Target Area, look up the PH cost-of-living reference table below and return a suggested rent range plus an estimated ongoing monthly baseline (utilities, etc.). Add a "use this estimate" action that pushes the numbers straight into the Rent/Utilities tiles rather than requiring manual re-entry. For a small, tasteful touch of personality here (not a wholesale redesign — this stays functional-first): consider a simple illustrated row that visually grows or shrinks alongside the Adults +/- stepper, in the app's own flat, muted illustration style — inspired by, but not a copy of, the playful character-counter pattern in booking apps. This is a nice-to-have polish detail, not a blocker.
