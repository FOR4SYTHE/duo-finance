...
**Perfect Adaptability (Old & New Upgrades):** All upgrades, whether modifying old components or building new ones, must always be 100% adaptable to *every* screen type, device type, and dimension (especially small phones). The UI must remain flawless without elements clipping, hiding behind navigation bars, or breaking aspect ratios. It all should be perfect.
...
**Unified Calendar & Bills System.** A card below the Monthly Report hero showing upcoming due dates and scheduled events. Tapping it opens a full unified calendar view that acts as the central hub for all time-based events in the app:
- **Bills & Budget Integration:** The couple can add or edit recurring or one-off due dates for bills (rent, tuition, subscriptions). Bills can be directly linked to specific Budget sub-categories (e.g., linking a "Meralco" bill to the "Electricity" sub-category under Utilities). When a bill is marked as **Paid**, the system will show an in-app confirmation prompt (e.g. "Log ₱X to [Category]?"). Upon confirmation, it logs an Expense Entry in the Spend Jar, which immediately deducts the amount from the connected budget category. Un-marking a paid bill will not auto-reverse the ledger entry, but will instead show a sync drift notice.
- **Cartify Scheduled Trips:** Scheduled grocery or shopping runs from Cartify appear directly on this calendar. Tapping a scheduled trip from the calendar instantly launches the active Cartify shopping session for that trip, complete with its pre-loaded budget and item list.
- **Insurance Renewals:** Policy renewal dates from the Insurance Tracker automatically populate the calendar.
- **Reminders Scope:** Each event can optionally trigger a reminder. **Scope the reminder honestly:** build in-app/in-browser notifications first (using the Web Notifications API while the app has permission and is open, plus a due-today/overdue banner on app open). True push notifications that arrive while the app is closed require a backend push service and service worker infrastructure. Build the honest v1 first and flag full push as a later phase.
...
- **Master Aggregation (The Main Ring):** Spend Jar's ring/total aggregates the *entire* expense entries ledger against the *entire* period budget (the hero-card total). The Jar's progress ring should move when Rent gets paid just as it does when an uncategorized coffee gets logged. It is NOT strictly for unallocated money; it represents the total velocity of all money leaving the household against the overarching budget.
- **Extra/discretionary allowance from unallocated budget:** Spend Jar reads the Budget Module's (4.2) "unallocated" figure for the active period — the gap between the hero-card target and what's assigned across category tiles. The couple sets what percentage of that unallocated amount (0–100%, default 0% — opt-in, never silently made spendable) becomes available as an *additional* flexible spending line item inside Spend Jar. This "Extra available: ₱X / RY" figure sits beside the jar's main total as its own separate line; it is a bonus layer, not a redefinition of what the jar tracks.
