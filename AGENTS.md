...
- **Move-in (one-time setup costs) vs. Smart Budget Estimator (ongoing monthly target) must produce genuinely different results — they currently render identically, which is a bug.** Both take Adults/Kids + Target Area as input, but:
  - **Move-in** outputs a one-time lump sum: estimated security deposit + advance rent + starter furniture/appliance costs for the initial move — a single upfront number, distinct from any recurring monthly figure.
  - **Smart Budget Estimator** outputs an ongoing *monthly* recurring target (rent + utilities + groceries baseline, etc.) — this is what can push directly into the hero card's Target Budget.
  Make sure these pull from different calculations against the cost-of-living table, not the same function with a different label.
- **Target Area coverage:** the current list is too narrow (Metro Manila only) and its dropdown is clipping (same "no fixed-height container" bug — fix per the Section 2 rule). Expand coverage nationwide using a hybrid approach: keep the small curated table as the fast, reliable default for common areas (Makati/BGC, Quezon City, etc.), and add a text-search field that falls back to a single on-demand Gemini call (gemini-3.1-flash-lite, same fix as the category-suggestion route) for any PH area typed in that isn't in the curated table — returning a rough estimate rather than nothing. Apply the same lessons from the earlier AI-suggestion bugs: verify the current model name, use `response.text` correctly (not as a function call), and fail gracefully to a clear "no estimate available for this area yet" state rather than a silent or generic error.
- **Inflation Outlook** depends only on the hero card's overall Target Budget — it does not need to factor in individual category allocations, since inflation affects overall purchasing power, not how the couple has chosen to split it. This is correct as currently built.
- **FX Volatility Buffer:** currently failing with a generic "Failed to fetch." Use the shared tool-card shell's proper error handling to surface the *actual* fetch error (bad URL, wrong date format, network issue, etc.) instead of a generic message — diagnose and fix the real cause, then confirm it against the shared error-handling pattern above.
