...
export function AmountInputModal({ isOpen, onClose, onConfirm, title, initialAmount = 0 }: AmountInputModalProps) {
    ...
    useEffect(() => {
        if (isOpen) {
            const initialInPrimary = isPhpPrimary ? initialAmount : initialAmount / exchangeRate;
            setDisplayValue(initialInPrimary > 0 ? initialInPrimary.toFixed(0) : "0");
        }
            ...
            {isOpen && (
                <div className="fixed inset-0 z-[110] flex items-center justify-center p-4" onClick={(e) => e.stopPropagation()}>
                    ...
                    <motion.div
                        ...
                        className="w-full max-w-md bg-[#111] rounded-[32px] border border-white/10 p-5 relative z-10 flex flex-col max-h-[90dvh] overflow-hidden will-change-transform"
                    >
