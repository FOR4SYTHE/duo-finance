...
export function Navigation() {
  ...
  return (
    <motion.div 
      ...
      animate={{ 
        y: isHiddenRoute ? 150 : 0, 
        opacity: isHiddenRoute ? 0 : 1 
      }}
      transition={{ type: "spring", stiffness: 300, damping: 30 }}
      className="fixed bottom-6 left-4 right-4 z-50 flex items-center justify-center gap-3"
      style={{ pointerEvents: isHiddenRoute ? 'none' : 'auto' }}
    >
      {/* Main Navigation Capsule — solid dark glass, no backdrop-blur */}
          ...
          {/* AI Corner Pill — Liquid Metal Restored */}
          <MetalFx preset="chromatic" variant="circle" strength={0.85}>
            <button
              disabled
